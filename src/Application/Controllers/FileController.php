...
class FileController
{
    ...
    private function isPathSafe(string $basePath, string $path): bool
    {
        ...
        if ($realBase) {
            ...
            return false; // Base path must exist
        }
        ...
        // 2. If path doesn't exist yet (e.g., for new file/dir), check its parent
        if ($realPath === false) {
        ...
        // 3. Ensure the path is within the base directory (prefix check)
        $baseWithSlash = rtrim($realBase, '/') . '/';
        if ($realPath !== $realBase && !str_starts_with($realPath, $baseWithSlash)) {
            return false;
        }

        // 4. Symlink Protection: Ensure no part of the path is an external symbolic link
        $relative = ltrim(substr($realPath, strlen($realBase)), '/');
