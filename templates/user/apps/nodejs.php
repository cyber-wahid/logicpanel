...
<div class="db-container">
    ...
    <script>
        ...
        modal.innerHTML = `
            <div style="background: var(--bg-card); border-radius: 8px; width: 100%; max-width: 900px; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 10px 40px rgba(0,0,0,0.5);">
                <div style="padding: 20px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
                    <h3 style="margin: 0; font-size: 18px; font-weight: 600;">
                        <i data-lucide="terminal" style="width: 20px; height: 20px; vertical-align: middle; margin-right: 8px;"></i>
                        Deployment Logs - ${serviceName}
                    </h3>
                    <button onclick="closeDeploymentLogs()" style="background: none; border: none; color: var(--text-color); font-size: 28px; cursor: pointer; padding: 0; line-height: 1; opacity: 0.7; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.7'">&times;</button>
                </div>
                <div style="flex: 1; overflow: hidden; position: relative;">
                    <div id="deployment-logs-content" style="
                        background: #0d1117;
                        color: #58a6ff;
                    ...
                    ">
                        <div style="color: #8b949e; animation: pulse 1.5s ease-in-out infinite;">
                            <i data-lucide="loader" style="width: 16px; height: 16px; vertical-align: middle; margin-right: 8px; animation: spin 1s linear infinite;"></i>
                            Fetching deployment logs...
                        </div>
                    </div>
                    <div id="deployment-status" style="
                        position: absolute;
                        top: 10px;
                        right: 10px;
                    ...
                    ">
                        ...
                        <span id="deployment-status-text">Deploying...</span>
                    </div>
                </div>
                <div style="padding: 16px 20px; border-top: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <button class="btn btn-secondary" onclick="closeDeploymentLogs()">Close</button>
                        <button class="btn btn-primary" onclick="refreshDeploymentLogs(${serviceId})" id="refresh-logs-btn">
                            <i data-lucide="refresh-cw" style="width: 16px; height: 16px; vertical-align: middle; margin-right: 4px;"></i>
                            Refresh
                        </button>
                    </div>
                </div>
        ...
        style.textContent = `
            ...
            #deployment-logs-content::-webkit-scrollbar {
                width: 8px;
            }
            #deployment-logs-content::-webkit-scrollbar-track {
                background: #161b22;
            }
            #deployment-logs-content::-webkit-scrollbar-thumb {
                background: #30363d;
                border-radius: 4px;
            }
            #deployment-logs-content::-webkit-scrollbar-thumb:hover {
                background: #484f58;
            }
        `;
    ...
    // Store previous logs and typing state for animation
    let previousLogs = '';
    let currentDisplayedLogs = '';
    let isTyping = false;

    // Typing animation function - makes logs appear character by character
    function typeText(element, newText, callback) {
        if (isTyping) return;
        isTyping = true;
        ...
        function typeLine() {
            ...
            function typeChar() {
                if (charIndex < line.length) {
                    currentDisplayedLogs += line[charIndex];
                    element.innerHTML = formatLogs(currentDisplayedLogs);
                    element.scrollTop = element.scrollHeight;
                    charIndex++;
                    
                    // Random typing speed for realistic effect (1-3ms per char)
                    setTimeout(typeChar, Math.random() * 2 + 1);
                } else {
                    // Add newline and move to next line
                    currentDisplayedLogs += '\n';
                    element.innerHTML = formatLogs(currentDisplayedLogs);
                    element.scrollTop = element.scrollHeight;
                    currentLine++;
                    
                    // Small delay between lines (10-30ms)
    ...
    function formatLogs(logs) {
        return logs
            ...
            .replace(/(error|failed|fail)/gi, '<span style="color: #ff7b72; font-weight: bold;">$1</span>')
            .replace(/(success|complete|done|✓)/gi, '<span style="color: #7ee787; font-weight: bold;">$1</span>')
            .replace(/(warning|warn)/gi, '<span style="color: #ffa657;">$1</span>')
            .replace(/(\$|>|#)/g, '<span style="color: #79c0ff;">$1</span>')
            .replace(/(npm|yarn|node|git|docker)/gi, '<span style="color: #f0883e;">$1</span>');
    }
        ...
        try {
            ...
            if (res.ok && data.logs) {
                ...
                if (logsContent) {
                    ...
                    if (newLogs !== previousLogs) {
                        if (!previousLogs) {
                            // First load - type everything with animation
                            currentDisplayedLogs = '';
                            typeText(logsContent, newLogs);
                        } else {
                            // Find new content and type it
                            const newContent = newLogs.substring(previousLogs.length);
                            if (newContent && !isTyping) {
                                typeText(logsContent, newContent);
                            }
                        }
                    ...
                    if (statusDiv && statusText) {
                        ...
                        if (data.logs.includes('=== Starting app ===') || data.logs.includes('Server running')) {
                            statusText.innerHTML = '<span style="color: #7ee787;">✓ Deployment Complete</span>';
                        ...
                        } else if (data.logs.includes('error') || data.logs.includes('failed')) {
                            statusText.innerHTML = '<span style="color: #ff7b72;">⚠ Deployment Error</span>';
                        } else {
                            statusText.innerHTML = '<span style="color: #58a6ff;">⟳ Deploying...</span>';
                        }
                    }
                }
            }
            ...
            const logsContent = document.getElementById('deployment-logs-content');
            if (logsContent) {
                logsContent.innerHTML = '<span style="color: #ff7b72;">Failed to fetch logs. Please try refreshing.</span>';
            }
        }
    }
    function refreshDeploymentLogs(serviceId) {
        const btn = document.getElementById('refresh-logs-btn');
        if (btn) {
            btn.disabled = true;
            btn.innerHTML = '<i data-lucide="loader" style="width: 16px; height: 16px; vertical-align: middle; margin-right: 4px; animation: spin 1s linear infinite;"></i> Refreshing...';
        }
        
        fetchDeploymentLogs(serviceId).then(() => {
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<i data-lucide="refresh-cw" style="width: 16px; height: 16px; vertical-align: middle; margin-right: 4px;"></i> Refresh';
                if (window.lucide) lucide.createIcons();
            }
        });
    }
    ...
    function closeDeploymentLogs() {
        ...
        // Reload apps list to show updated status
        loadApps();
    }
    </script>
