...
<script>
    ...
    function loginToAdminer(driver, host, user, encryptedPass, db) {
        ...
        // Adminer URL format for PgSQL: https://db.domain/?pgsql=HOST&username=USER&db=NAME
        const domain = window.location.hostname;
        const adminerUrl = `https://db.${domain}/?pgsql=${host}&username=${user}&db=${db}`;
                ...
                await showCustomAlert({
                    ...
                    html: `<div style="text-align:left;font-size:13px;background:#f3f4f6;padding:12px;border-radius:6px;border:1px solid #e5e7eb;margin-bottom:12px">
                           ...
                           <div style="margin-top:15px; text-align:center;">
                                <a href="<?= $base_url ?? '' ?>/public/adminer.php?pgsql=localhost&username=postgres&db=${data.database.name}" 
                                   target="_blank" 
