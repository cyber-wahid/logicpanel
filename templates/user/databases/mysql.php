...
<script>
                ...
                list.innerHTML = filtered.map(db => `
                    <tr>
                        <td>
                            ...
                            <div style="font-size:11px;color:var(--text-secondary);">${(db.host === 'lp-mysql-mother' || db.host === 'lp-postgres-mother' || db.host === 'lp-mongo-mother') ? window.location.hostname : db.host}:${db.port}</div>
                        </td>
                        <td><span style="color:var(--text-secondary);">-</span></td>
                        <td><span class="user-badge">${db.user}</span></td>
                        <td class="actions-col">
                            <div style="display:flex;gap:5px;justify-content:flex-end;">
                                <a href="${window.location.protocol}//${window.location.hostname}:7777/public/adminer.php?server=${db.host}&username=${db.user}&db=${db.name}" target="_blank" class="btn btn-sm btn-secondary" title="Login to Adminer">
    ...
    function loginToAdminer(driver, host, user, encryptedPass, db) {
        ...
        const adminerUrl = `${window.location.protocol}//${window.location.hostname}:7777/public/adminer.php?server=${host}&username=${user}&db=${db}`;
                ...
                await showCustomAlert({
                    ...
                    html: `<div style="text-align:left;font-size:13px;background:#f3f4f6;padding:12px;border-radius:6px;border:1px solid #e5e7eb;margin-bottom:12px">
                           ...
                           <div style="margin-top:15px; text-align:center;">
                                 <a href="${window.location.protocol}//${window.location.hostname}:7777/public/adminer.php?server=${data.database.host}&username=${data.database.user}&db=${data.database.name}" 
                                   target="_blank" 
